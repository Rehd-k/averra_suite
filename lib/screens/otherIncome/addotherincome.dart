...
class AddOtherIncomeScreen extends StatefulWidget {
  ...
  const AddOtherIncomeScreen({super.key, this.updateInfo});
...
class _AddOtherIncomeScreenState extends State<AddOtherIncomeScreen> {
  ...
  Future<void> createNewIncome() async {
    ...
    if (widget.updateInfo == null) {
      await apiService.post('otherIncome', data);
    } else {
      await apiService.patch('otherIncome/${widget.updateInfo?['_id']}', data);
    }
  ...
  Future<void> clearIncomeForm() async {
    setState(() {
      _category = '';
